defmodule StarView.Utility.StreamRegistry do
  ...
  Add to your application's supervision tree (StarView does not start
  ...
  In your stream controllers, replace `StarView.start(conn)` with
  `StarView.start_stream/2` (or call this module directly):

      def stream(conn, _params) do
        scope = conn.assigns.current_scope

        conn = StarView.start_stream(conn, scope.user.id)

        loop(conn, state)
      end

  If no `tabId` signal is present in the request, falls back to
  `StarView.start/1` without deduplication — so existing streams
  keep working while you roll out the client-side signal.

  Adapted from [dstar](https://github.com/RicoTrevisan/dstar) (MIT).
  """
  ...
  current process, and calls `StarView.start/1`.

  If no `tabId` signal is present, falls back to `StarView.start/1`
  ...
  def start_stream(conn, scope_key) do
    signals =
      case StarView.Signals.read(conn) do
        {:ok, signals} -> signals
        {:error, _} -> %{}
      end
