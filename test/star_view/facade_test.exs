defmodule StarView.FacadeTest do
  use ExUnit.Case, async: true

  import Plug.Test

  alias StarView.Signals.ReadError
  ...
  describe "read_signals/1 and read_signals!/1" do
    test "read_signals/1 returns a bare signal map" do
      ...
      assert StarView.read_signals(conn) == %{"count" => 1}
    end

    test "read_signals!/1 matches read_signals/1 on valid payloads" do
      conn = conn(:post, "/", ~s({"count":1}))

      assert StarView.read_signals!(conn) == StarView.read_signals(conn)
    end

    test "read_signals/1 raises ReadError on invalid JSON" do
      conn = conn(:post, "/", "not-json")

      assert_raise ReadError, fn -> StarView.read_signals(conn) end
    end
  end
  ...
  describe "start/1 and check_connection/1" do
    ...
    test "check_connection/1 returns ok on a started connection" do
      ...
      assert {:ok, conn} = StarView.check_connection(conn)
      assert conn.state == :chunked
    end
    ...
    test "check_connection/1 returns error when SSE was not started" do
      ...
      assert {:error, ^conn} = StarView.check_connection(conn)
    end
  end
  ...
  describe "patch and remove helpers" do
    test "patch_signals/2 emits a signal patch event" do
      body =
        started_conn()
        |> StarView.patch_signals(%{count: 2})
        |> sse_body()
    ...
    test "patch_signals_raw/2 emits pre-encoded JSON" do
      body =
        started_conn()
        |> StarView.patch_signals_raw(~s({"count":3}))
        |> sse_body()
    ...
    test "patch_elements/2 emits an element patch event" do
      body =
        started_conn()
        |> StarView.patch_elements(~s(<motion.div id="x">1</motion.div>), selector: "#x")
        |> sse_body()
    ...
    test "remove_elements/2 emits a remove patch" do
      body =
        started_conn()
        |> StarView.remove_elements("#flash")
        |> sse_body()
    ...
    test "remove_signals/2 emits null paths for removed signals" do
      body =
        started_conn()
        |> StarView.remove_signals(["user.email", "user.name"])
        |> sse_body()
  ...
  describe "script helpers" do
    test "execute_script/2 appends a script to body" do
      body =
        started_conn()
        |> StarView.execute_script("console.log('facade')")
        |> sse_body()
    ...
    test "console_log/2 emits a console call" do
      body =
        started_conn()
        |> StarView.console_log(%{debug: true})
        |> sse_body()
  ...
  describe "send/3" do
    test "sends a raw SSE event through the facade" do
      body =
        started_conn()
        |> StarView.send("custom-event", ["line one", "line two"], event_id: "e1")
        |> sse_body()
