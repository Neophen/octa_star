defmodule StarView.SDKTestPlug do
  @moduledoc false
  ...
  def call(%Plug.Conn{request_path: "/test"} = conn, _opts) do
    case StarView.read_signals(conn) do
      %{"events" => events} ->
        Enum.reduce(events, StarView.start(conn), fn event, conn ->
    ...
    error in StarView.Signals.ReadError ->
  ...
  defp dispatch_event(conn, %{"type" => "patchElements"} = event) do
    StarView.patch_elements(conn, Map.get(event, "elements"), event_opts(event, :elements))
  end
  ...
  defp dispatch_event(conn, %{"type" => "patchSignals"} = event) do
    ...
    case event do
      %{"signals-raw" => signals} -> StarView.patch_signals_raw(conn, signals, opts)
      %{"signals" => signals} -> StarView.patch_signals(conn, signals, opts)
    end
  end

  defp dispatch_event(conn, %{"type" => "executeScript"} = event) do
    StarView.execute_script(conn, Map.fetch!(event, "script"), event_opts(event, :script))
  end
